...
def upload_file(http_client: HTTPClient, file_path: str) -> str:
    ...
    if not isinstance(upload_info, dict):
        raise ValueError("Unexpected upload response format")
...
def download_artifacts(
    ...
    try:
        ...
        if not isinstance(artifacts, list):
            renderer.print_error("Unexpected artifact response format")
            return
...
def run_command(
    ...
    try:
        ...
        if not isinstance(run_info, dict):
            renderer.print_error("Unexpected run response format")
            return 1
        ...
        if wait or follow:
            ...
            while True:
                ...
                if not isinstance(run_status, dict):
                    renderer.print_error("Unexpected run status format")
                    return 1
            ...
            if not isinstance(final_status, dict):
                renderer.print_error("Unexpected final status format")
                return 1
