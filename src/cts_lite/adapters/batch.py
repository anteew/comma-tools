...
class BatchAdapter(ABC):
    ...
    @abstractmethod
    def build_command(self, params: Dict[str, Any], inputs: List[InputRef], work_dir: Path) -> List[str]:
...
class CruiseControlAnalyzerAdapter(BatchAdapter):
    ...
    def build_command(self, params: Dict[str, Any], inputs: List[InputRef], work_dir: Path) -> List[str]:
        ...
        cmd = [
            sys.executable, "-m", "comma_tools.analyzers.cruise_control_analyzer",
            log_file,
            "--speed-min", str(params.get("speed_min", 55.0)),
            "--speed-max", str(params.get("speed_max", 56.0)),
            "--marker-type", params.get("marker_type", "blinkers"),
            "--marker-pre", str(params.get("marker_pre", 5.0)),
            "--marker-post", str(params.get("marker_post", 5.0)),
            "--marker-timeout", str(params.get("marker_timeout", 10.0)),
        ]
...
class RlogToCsvAdapter(BatchAdapter):
    ...
    def build_command(self, params: Dict[str, Any], inputs: List[InputRef], work_dir: Path) -> List[str]:
        ...
        cmd = [
            sys.executable, "-m", "comma_tools.analyzers.rlog_to_csv",
            "--rlog", log_file,
            "--out", str(output_file),
        ]
...
class CanBitwatchAdapter(BatchAdapter):
    ...
    def build_command(self, params: Dict[str, Any], inputs: List[InputRef], work_dir: Path) -> List[str]:
        ...
        cmd = [
            sys.executable, "-m", "comma_tools.analyzers.can_bitwatch",
            "--csv", csv_file,
