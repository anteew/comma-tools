...
Note: Drives are retained 3 days (or 1 year with comma prime)
    ...
    input_group.add_argument(
        "--route", help="Canonical route name (dongle_id|YYYY-MM-DD--HH-MM-SS)"
    )
    input_group.add_argument("--url", help="Connect URL (https://connect.comma.ai/dongle/slug)")
    ...
    parser.add_argument(
        "--parallel", type=int, default=4, help="Number of parallel downloads (default: 4)"
    )
    parser.add_argument(
        "--no-resume", action="store_true", help="Disable resume of partial downloads"
    )

    parser.add_argument("--since", help="Search start date (YYYY-MM-DD) for connect URL resolution")
    parser.add_argument("--until", help="Search end date (YYYY-MM-DD) for connect URL resolution")
    parser.add_argument(
        "--days",
        type=int,
        default=7,
...
def print_report(report: DownloadReport, args: argparse.Namespace) -> None:
    ...
    if args.json:
        ...
        print(
            f"  Skipped: {len(report.skipped_paths)} files ({format_bytes(report.skipped_bytes)})"
        )
