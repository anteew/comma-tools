...
class TestErrorHandling:
    ...
    async def test_tool_timeout_handling(self, execution_engine, sample_run_context):
        ...
        with patch("asyncio.wait_for") as mock_wait_for:
            ...
            with patch.object(
                execution_engine.resource_manager, "cleanup_run_resources"
            ) as mock_cleanup:
        ...
        assert (
            "timeout" in sample_run_context.error.lower()
            or "timed out" in sample_run_context.error.lower()
        )
    ...
    async def test_system_error_recovery(self, execution_engine, sample_run_context):
        ...
        with patch.object(execution_engine, "_execute_tool_sync") as mock_sync:
            ...
            with patch.object(
                execution_engine.recovery_manager, "attempt_recovery"
            ) as mock_recovery:
...
class TestRecoveryManager:
    ...
    async def test_transient_system_error_recovery(self, recovery_manager, sample_run_context):
        ...
        sample_run_context.error_details = {"system_error": "Connection temporarily unavailable"}
    ...
    def test_error_fix_suggestions(self, recovery_manager):
        ...
        suggestion = recovery_manager.suggest_tool_fix(
            RuntimeError("Tool failed"), stderr="MemoryError: out of memory"
        )
        assert "memory" in suggestion.lower()

        # Test permission error
        suggestion = recovery_manager.suggest_tool_fix(
            PermissionError("Access denied"), stderr="Permission denied"
        )
        assert "permission" in suggestion.lower()

        # Test file not found
        suggestion = recovery_manager.suggest_tool_fix(
            FileNotFoundError("File not found"), stderr="file not found"
        )
        assert "missing" in suggestion.lower() or "not found" in suggestion.lower()

        # Test timeout
        suggestion = recovery_manager.suggest_tool_fix(
            TimeoutError("Operation timed out"), stderr=""
        )
