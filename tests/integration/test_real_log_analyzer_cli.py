...
def test_cli_default_parse(real_log_path):
    proc = _run([str(real_log_path), "--install-missing-deps"])
...
def test_cli_marker_none(real_log_path):
    proc = _run([str(real_log_path), "--marker-type", "none", "--install-missing-deps"])
...
def test_cli_with_repo_root_if_openpilot_present(real_log_path):
    ...
    proc = _run([str(real_log_path), "--repo-root", str(parent), "--install-missing-deps"])
    assert proc.returncode == 0, proc.stderr
