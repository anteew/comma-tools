...
class TestConnectCLI:
    ...
    def test_parser_route_argument(self):
        ...
        args = parser.parse_args([
            '--route', 'dcb4c2e18426be55|2024-04-19--12-33-20',
            '--logs'
        ])
    ...
    def test_parser_file_type_options(self):
        ...
        parser = create_parser()
        args = parser.parse_args([
            '--route', 'test|2024-01-01--00-00-00',
            '--logs', '--qlogs', '--cameras', '--dcameras', '--ecameras', '--qcameras'
        ])
    ...
    def test_main_missing_auth(self, mock_load_auth):
        ...
        with patch('sys.argv', ['comma-connect-dl', '--route', 'test|2024-01-01--00-00-00', '--logs']):
            with pytest.raises(SystemExit) as exc_info:
                main()
